...
public class Turtle {
	 ...
	 *             if the starting position is not in the square with corners (0.0,
	 *             0.0) and (1.0, 1.0)
	 */
	...
	public Turtle(Turtle other) {
		this.position = new Point2(other.position); // this is defensive copy
		this.direction = new Direction2(other.direction.getAngle());
		this.pen = new Pen(other.pen);
		this.commands = new ArrayList<TurtleCommand>(other.commands);
	...
	public void walk(double distance) {
		...
		// make new turtlecommandwalk object to represent the action
		TurtleCommand s = new TurtleCommandWalk(this, distance);
		this.commands.add(s);
	}
	...
	public void turnLeft(double delta) {
		...
		// make new tutrlte commnad object to define the action
		TurtleCommand x = new TurtleCommandTurnLeft(this, delta);
		this.commands.add(x);
	}
	...
	public void turnRight(double delta) {
		...
		// make new turltecommnad object to define the action
		TurtleCommand xx = new TurtleCommandTurnRight(this, delta);
		this.commands.add(xx);
	}
	...
	public void turnTo(double angle) {
		...
		// make new turltecommnad object to define the action
		TurtleCommand xix = new TurtleCommandTurnTo(this, angle);
		this.commands.add(xix);
	...
	public void penOn() {
		...
		// make new turltecommnad object to define the action
		TurtleCommand xvx = new TurtleCommandPenOn(this);
		this.commands.add(xvx);
	...
	public void penOff() {
		...
		// make new turltecommnad object to define the action
		TurtleCommand xtx = new TurtleCommandPenOff(this);
		this.commands.add(xtx);
	}
	...
	public void setPenColor(Color c) {
		...
		// make new turltecommnad object to define the action
		TurtleCommand xax = new TurtleCommandSetPenColor(this, oldColor, c);
		this.commands.add(xax);
	 ...
	 */
	public Pen getPen() {
		return new Pen(this.pen); // defensive copy
	...
	public List<TurtleCommand> getCommands() {
		List<TurtleCommand> res = new ArrayList<TurtleCommand>(this.commands);
		return res;
	}
	 ...
	 * 
	 * @param n
	 *            the number of times to repeat the commands that this turtle has
	...
	public void repeatCommands(int n) {
		List<TurtleCommand> repeat = new ArrayList<TurtleCommand>(this.commands);
		for (int i = 0; i < n; i++) {
			// we need another for loop to go over the list to look for
			// commands.
			for (int j = 0; j < repeat.size(); j++) {
				repeat.get(j).execute();
			}
		}
	}
	 ...
	 * 
	 * @param commands
	 *            a list of commands for this turtle to execute
	 */
	public void doCommands(List<TurtleCommand> commands) {
		for (int i = 0; i < commands.size(); i++) {
			// this denotes the entire current class to execute the commands
			commands.get(i).execute(this);
		}
	}
