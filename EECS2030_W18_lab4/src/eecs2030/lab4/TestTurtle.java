...
public class TestTurtle {
	...
	@Rule
	public Timeout globalTimeout = Timeout.seconds(3); // raise time to 2 sec due to mac speed

	@Test
	public void test01_copyCtor() {
		Turtle t = new Turtle(0.1, 0.2, 30.0, Color.CYAN);
		Turtle u = new Turtle(t);
		assertNotSame("copy ctor failed to make a defensive copy of other.position", u.leakPosition(),
				t.leakPosition());
		assertEquals("copy ctor failed to correctly copy other.position", new Point2(0.1, 0.2), u.leakPosition());
	}
	...
	public void test02_copyCtor() {
		Turtle t = new Turtle(0.1, 0.2, 30.0, Color.CYAN);
		Turtle u = new Turtle(t);
		assertNotSame("copy ctor failed to make a defensive copy of other.direction", u.leakDirection(),
				t.leakDirection());
		assertEquals("copy ctor failed to correctly copy other.direction", new Direction2(30.0), u.leakDirection());
	}
	...
	public void test03_copyCtor() {
		Turtle t = new Turtle(0.1, 0.2, 30.0, Color.CYAN);
		Turtle u = new Turtle(t);
		assertNotSame("copy ctor failed to make a defensive copy of other.pen", u.leakPen(), t.leakPen());
		assertEquals("copy ctor failed to correctly copy other.pen", new Pen(Color.CYAN), u.leakPen());
	}
	...
	public void test04_copyCtor() {
		...
		Turtle u = new Turtle(t);
		assertEquals("copy ctor failed to correctly copy other.position", new Point2(0.15, 0.25), u.leakPosition());
		assertEquals("copy ctor failed to correctly copy other.direction", new Direction2(35.0), u.leakDirection());
		assertEquals("copy ctor failed to correctly copy other.pen", new Pen(Color.RED), u.leakPen());
	}

	@Test
	public void test05_getPosition() {
		Turtle t = new Turtle(0.25, 0.75, 35.0, Color.RED);
		assertNotSame("getPosition failed to make a defensive copy of this.position", t.leakPosition(),
				t.getPosition());
		assertEquals("getPosition returned the wrong position", new Point2(0.25, 0.75), t.getPosition());
	}

	@Test
	public void test06_getDirection() {
		Turtle t = new Turtle(0.25, 0.75, 45.0, Color.RED);
		assertNotSame("getDirection failed to make a defensive copy of this.direction", t.leakDirection(),
				t.getDirection());
		assertEquals("getDirection returned the wrong direction", new Direction2(45.0), t.getDirection());
	}

	@Test
	public void test07_getPen() {
		Turtle t = new Turtle(0.25, 0.75, 45.0, Color.MAGENTA);
		assertNotSame("getPen failed to make a defensive copy of this.pen", t.leakPen(), t.getPen());
		assertEquals("getPen returned the wrong pen", new Pen(Color.MAGENTA), t.getPen());
	}

	@Test
	public void test08_getCommands() {
		Turtle t = new Turtle(0.25, 0.75, 45.0, Color.MAGENTA);
		assertNotSame("getCommands failed to make a defensive copy of this.commands", t.leakCommands(),
				t.getCommands());
	}
	...
	public void test09_getCommands() {
		Turtle t = new Turtle(0.25, 0.75, 45.0, Color.MAGENTA);
		assertEquals("getCommands returned the wrong list of commands", new ArrayList<TurtleCommand>(),
				t.getCommands());
	}
	...
	private static void testCommands(List<TurtleCommand> exp, List<TurtleCommand> got) {
		assertEquals("getCommands returned a list with the wrong size", exp.size(), got.size());
		for (int i = 0; i < exp.size(); i++) {
			String err = String.format("getCommands command %d, expected %s, got %s", i,
					TestTurtle.toString(exp.get(i)), TestTurtle.toString(got.get(i)));
			assertTrue(err, TestTurtle.equals(exp.get(i), got.get(i)));
		}
	}
	...
	private static boolean equals(TurtleCommand a, TurtleCommand b) {
		...
		if (a instanceof TurtleCommandTurnLeft) {
			...
			return x.getOldColor().equals(y.getOldColor()) && x.getNewColor().equals(y.getNewColor());
		}
		return false;
	}
